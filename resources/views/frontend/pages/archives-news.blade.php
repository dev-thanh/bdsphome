							...
							<div class="tab-item id-category-news" id="tab{{$k+1}}" data-cate="{{$value->id}}">
								<div class="tab__new page__news-group">
									@if(!empty($value->Posts))
										...
										<?php $news = $value->Posts()->paginate(10); ?>

										@foreach($news as $val)
										<div class="new">
											<a href="{{route('home.news-single',['slug'=>$val->slug])}}" class="frame">
											...
											<div class="new__content">
												...
												<h3 class="new__title">
													<a href="{{route('home.news-single',['slug'=>$val->slug])}}" title="{{$val->name}}">
														{{$val->name}}
													</a>
												</h3>
												<div class="new__desc">
												...
												<a href="{{route('home.news-single',['slug'=>$val->slug])}}" class="btn btn__global" title="Chi tiết">
													Chi tiết
												</a>
											</div>
										</div>
        ...
        jQuery(document).ready(function($) {
			...
			$(document).on('click','ul.pagination .li-item:not(".active,.next-page,.prev-page") a',function(e) {
	        	...
	        	$.ajax({
		            ...
		            url: '{{route('home.ajax-load-news')}}?page='+next_page,
	        ...
	        $(document).on('click','.prev-page',function(e) {
	        	...
	        	$.ajax({
		            ...
		            url: '{{route('home.ajax-load-news')}}?page='+prev_page,
